...
- **MEASURED (build, 2026-09-09):** the vial keymap builds and links with
  `MAXTOUCH_DEBUG` left at **`no`** — the tunnel is guarded on `VIA_ENABLE`
  alone — producing a 177,152-byte `foodforarabbit_sofle_procyon_wt_vial.uf2`
  from a clean `vial-procyon`. Vial and live sensor tuning coexist in one
  firmware with **zero** changes to vial-qmk.
  **CORRECTED 2026-09-09 — withdrawn text kept for audit:** this bullet
  previously claimed the keymap "builds **with** `MAXTOUCH_DEBUG=yes`" via
  "the tunnel patch below". That was true only of the abandoned driver-patch
  branch. With the current keymap-side tunnel, `MAXTOUCH_DEBUG=yes` still
  fails to link, exactly as the bullet above it says.
...
> **CORRECTED 2026-09-09. This section described the DRIVER-PATCH approach,
> which was abandoned. The withdrawn text is preserved at the end of the
...
1. **vial-qmk carries ONE commit as of 2026-09-10** — branch
   ...
   The debug protocol is reimplemented in our keymap instead, so the fork
   keeps zero divergence. `origin/debug-vial-tunnel` survives as a record of
   the abandoned approach; **do not merge it**, though it does carry a real
   upstream bounds-check fix worth sending to george-norton as a PR.
2. **This repo, branch `trackpad-tuning`**: `keymaps/vial/keymap.c` carries a
   `raw_hid_receive_kb()` tunnel — the host wraps a debug packet as
   `[0x4D, <packet>]` and the reply comes back with the same prefix.
   **Compiled under `#if defined(VIA_ENABLE)` ALONE** (keymap.c:175).
   `MAXTOUCH_DEBUG` is NOT required and must stay `no`.
3. **Build symlink:** `vial-qmk/keyboards/foodforarabbit/sofle_procyon_wt`
   ...
   MEASURED 2026-09-09: links clean, produces a 177,152-byte
   `foodforarabbit_sofle_procyon_wt_vial.uf2`, and `arm-none-eabi-nm` on the
   `.elf` shows `raw_hid_receive_kb` as a strong symbol at `0x10002ae4` — so
   the tunnel really is in the image. No `MAXTOUCH_DEBUG=yes` on the command
   line, and no branch switch in vial-qmk.

**WITHDRAWN 2026-09-09 — kept verbatim for audit, DO NOT ACT ON.** This
section previously instructed building from vial-qmk branch
`debug-vial-tunnel` with `MAXTOUCH_DEBUG=yes`, describing a patched
`maxtouch.c` that exposed `maxtouch_debug_hid_receive()`. That reintroduces
the fork divergence Ryan explicitly rejected. `START-HERE.md` already carried
the correct story; PLAN.md had not been updated — and PLAN.md is the file that
claims to win on disagreement, which is exactly why the correction lives here
and not only in START-HERE.
...
1. `keymaps/vial/rules.mk`: add `CONSOLE_ENABLE = yes` (for item 4 below).
   **CORRECTED 2026-09-09.** This item previously read "add
   `MAXTOUCH_DEBUG = yes` (tunnel already in keymap.c — done on this
   branch)". That instruction is WITHDRAWN and kept visible here so the
   change is auditable: the tunnel is guarded on `VIA_ENABLE` ALONE
   (keymap.c:175), and `MAXTOUCH_DEBUG = yes` re-triggers the
   `multiple definition of 'raw_hid_receive'` link failure this whole design
   exists to avoid. It must stay `no`.
...
winning curve values become the new compiled defaults in keymap.c.
**CORRECTED 2026-09-09** — this previously read "Optionally set
`MAXTOUCH_DEBUG = no` again if you want the debug surface gone", which
presumed flash 1 had set it to `yes`. It never did and never will. To retire
the debug surface you delete or `#if 0` the tunnel block in `keymap.c`
(the `#if defined(VIA_ENABLE)` section), not a rules.mk flag. REASONED: leave
the tunnel in until S1/S2 are confirmed dead across a week of use — it costs
a little flash space and keeps the instrument available for free.
